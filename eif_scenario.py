...
class Dataset:
    ...
    rotation: float = 0.0      # Drehung der Merkmale (0 = keine, 1 = ROTATION_MAX_DEG Grad in jeder Ebene aus zwei aufeinanderfolgenden Merkmalen)
    ...
    def names(self):
        ...
        if self.rotation > 0:
            names = [f"{n} (gedreht)" for n in names]
...
def rotate_features(X, rotation):
    """Dreht den Merkmalsraum: je zwei aufeinanderfolgende Merkmale (0/1, 2/3, ...) werden in standardisierten Einheiten um den Winkel rotation * ROTATION_MAX_DEG gedreht (ein ungerades letztes bleibt), dann zurück
    in die ursprüngliche Größenordnung. Orthogonal und affin: Mahalanobis-Abstände und die MCD ändern sich nicht, achsenparallele Schnitte schon."""
    mu, sd = X.mean(axis=0), X.std(axis=0)
    Z = (X - mu) / sd
    theta = np.radians(rotation * C.ROTATION_MAX_DEG)
    c, s = np.cos(theta), np.sin(theta)
    out = Z.copy()
    for i in range(0, Z.shape[1] - 1, 2):
        out[:, i] = c * Z[:, i] - s * Z[:, i + 1]
        out[:, i + 1] = s * Z[:, i] + c * Z[:, i + 1]
    return mu + out * sd
...
def generate_dataset(n_tours, p, n_modes, curvature, noise, contamination_pct, kind, strength, seed, n_noise=0, rotation=0.0):
    ...
    if p > C.N_BASE_FEATURES:
        ...
        X = X[:, :p]
    if rotation > 0:
        X = rotate_features(X, rotation)
    if n_noise > 0:
        X = np.concatenate([X, C.EXTRA_MEAN + C.EXTRA_SCALE * noise_columns[:, :n_noise]], axis=1)
    return Dataset(X=X, z=z_used, anomaly=anomaly, mode=mode, n_modes=int(n_modes), kind=kind, p=int(p), n_noise=int(n_noise), rotation=float(rotation))
